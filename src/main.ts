...
const onInteractionCreate = async (interaction: Interaction<CacheType>) => {
    ...
    if (interaction.commandName === Suretate.data.name) await onSuretate(interaction);
}
...
const onModalCreate = async (interaction: Interaction<CacheType>) => {
    ...
    if (interaction.customId === Suretate.data.name) await onSuretateModal(interaction);
}
